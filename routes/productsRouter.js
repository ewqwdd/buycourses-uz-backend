...
router.post("/buy", authMiddleware, async (req, res) => {
    try {
        ...
        await Transaction.create({ userId: id, productId, amount: product.price, type: 'buy' });
        await Transaction.create({ userId: product.userId, amount: product.price, productId, type: 'sell' });
        res.json(product);
    } catch (error) {
        console.error("Unable to connect to the database:", error);
        res.status(500).json({ message: "Ошибка сервера" });
